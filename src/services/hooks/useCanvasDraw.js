...
export const useCanvasDraw = (elements, setElements, pressedKeys= new Set()) => {
    ...
    const handleStart = (event) => {
      ...
      const handleMove = (event) => {
       ...
       if (action === "erasing") {
        ...
        updateElement(
            index, x1, y1, clientX, clientY, "pencil",
            { color: backgroundColor }, elements, setElements, lineThickness
        );
        return;
        }
        if (action === "panning") {
        ...
        if (action === "drawing") {
          const index = elements.length - 1;
          const { x1, y1 } = elements[index];
          updateElement(index, x1, y1, clientX, clientY, tool, { color }, elements, setElements, lineThickness);
